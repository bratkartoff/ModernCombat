...
protected func Initialize()
{
  ...
  Sound(Format("ClonkDie%d.ogg", RandomX(1,26)));
  aGrenades = [];
...
public func SetFakeDeathEffects(object pClonk, bool fNoScreenRGB, int iTime)
{
  ...
  //Sichtwerte für den Schwerverletzten setzen
...
{
  if(GetEffectCount("IntFakeDeathMenu", this) > 0)
  {
    //Unmittelbar nach dem Tod kein Inventar-Menü erlauben
    var timer = GetEffect("IntFakeDeathMenu", this, 0, 6);
    if(timer <= 1)
    {
      DoMenu();
      return true;
...
public func ControlUp(object pCaller)
{
  ...
  //Defibrillator des Anfassenden verwenden, wenn nicht vorhanden: Schwerverletzten nach einem durchsuchen
...
public func CustomHUD(int source)
{
  ...
  else if(source == HUD_Contained)
  {
    return GetEffect("IntFakeDeathEffectsData", clonk);
  }

  return false;
}

public func UpdateHUD(object pHUD, int source)
{
  var remainingTime = 1 + TimeLeft() / 35;
  if(source == HUD_Grabbed)
  {
    pHUD->Ammo(remainingTime, 0, Format("%s", GetPlayerName(clonk->GetOwner())), false, 0, 0, source);
  }
  else if(source == HUD_Contained)
  {
    pHUD->HUDMessage(Format("$DeathCounter$", remainingTime), source, true);
  }
