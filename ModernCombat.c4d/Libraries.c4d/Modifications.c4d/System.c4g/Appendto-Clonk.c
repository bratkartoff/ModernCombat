...
protected func Initialize()
{
  ...
  //Schwerverletzteneffekt nur wenn spielerkontrolliert
...
protected func Hurt()
{
  HurtSounds(9, LastDamageType());
  return 1;
}

public func CancelVoiceSounds()
{
  Sound("ClonkAction*.ogg", false, 0, 0, 0, -1);
  Sound("ClonkBurn*.ogg", false, 0, 0, 0, -1);
  Sound("ClonkCough*.ogg", false, 0, 0, 0, -1);
  Sound("ClonkDie*.ogg", false, 0, 0, 0, -1);
  Sound("ClonkMelee*.ogg", false, 0, 0, 0, -1);
  Sound("ClonkPain*.ogg", false, 0, 0, 0, -1);
  Sound("ClonkPoisened*.ogg", false, 0, 0, 0, -1);
  Sound("ClonkSmallBurn*.ogg", false, 0, 0, 0, -1);
  Sound("ClonkSmallPain*.ogg", false, 0, 0, 0, -1);
  Sound("ClonkPainMelee*.ogg", false, 0, 0, 0, -1);
...
func Hit2(int xDir, int yDir)
{
  ...
  if(!FindObject(FDMG) || hit <= 800) return _inherited(xDir,yDir,...);
  {
    ...
    Sound("ClonkImpactDamage*.ogg",0,0,0,GetOwner()+1);
  }
...
protected func DoPoints()
{
  ...
  {  
    ...
    DoPlayerPoints(BonusPoints("Kill"), RWDS_BattlePoints, killer, GetCursor(killer), IC01,"RWDS_PointsKill.ogg");
...
global func FakeDeath(object pTarget)
{
  ...
  //Schwerverletztenobjekt erstellen
...
func Death2()
{
  ...
  //Todesanimation überspringen wenn kriechend gestorben
...
public func DeathSound(object pTarget)
{
  ...
  //Todesschrei
  Sound(Format("ClonkDie%d.ogg", RandomX(1,26)), false, pTarget);
  CancelVoiceSounds();
}
