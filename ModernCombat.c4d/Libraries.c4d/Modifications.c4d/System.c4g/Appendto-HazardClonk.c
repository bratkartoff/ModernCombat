...
public func SetCrosshairIndicator(int iTime)
{
  //Fahrzeugfadenkreuze
  for(var obj in FindObjects(Find_And(Find_ID(VCRH), Find_Owner(GetOwner()))))
    obj->~SetIndicator(iTime);

  if(crosshair)
  {
    crosshair->~SetIndicator(iTime);
    return true;
  }
  return false;
}
...
private func TestSpread()
{
  ...
  if(proc == "WALK")
  {
    var fMidWalk = GetComDir() != COMD_None && GetComDir() != COMD_Down;
    if(fMidWalk && spread < CH_WalkSpread)
...
public func UpdateCharge(bool fForceUpdate)
{
  ...
  if(contained && (contained->~IsWeapon() || contained->~CustomHUD(HUD_Contained)))
  {
    Content = contained;
    source = HUD_Contained;
  }
  ...
  {
    Content = pHUDTarget;
    source = HUD_SpecialTarget;
  }

  //HUD
  var hud = GetHUD();
  if(hud) hud->Update(Content, AmmoStoring(),this, fForceUpdate, source);
...
{
    //Inventar wird als Schwerverletzter niemals manipuliert
    if(IsFakeDeath(this))
      return;
...
public func StoreGrenade(object pGrenade)
{
  ...
  HelpMessage(GetOwner(),"$Collected$",this,GetID(pGrenade));
  //Nachschubinfo: Handgranate aufgenommen
  ResupplyInfo(this,GetID(pGrenade),0,1);
...
protected func RejectCollect(id idObj, object pObj)
{
  ...
  if(IsFakeDeath(this) && Contained(pObj) == Contained(this))
    //Holen aus dem eigenen Schwerverletzten-Objekt in das Clonk-Inventar verbieten
    return 1;
  ...
  if(pObj ->~ IsWeapon())
  {
    ...
    {
      //Waffe des selben Typs bereits vorhanden oder Waffenlimit erreicht?
