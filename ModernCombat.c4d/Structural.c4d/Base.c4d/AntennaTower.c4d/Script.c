...
public func Destructable()	{return destructability;}
...
{
  //Bereits zerstört oder kein Schaden: Nichts weiter unternehmen
  if(IsDestroyed() || change < 0)
    return false;
  //Seilhalterungen vorhanden, per Regel deaktiviert oder unzerstörbar: Schaden nullifizieren
  if(WorkingRopesCount() || NoScenarioEvents() || !destructability)
  {
    DoDamage(-GetDamage());
    return false;
  }
