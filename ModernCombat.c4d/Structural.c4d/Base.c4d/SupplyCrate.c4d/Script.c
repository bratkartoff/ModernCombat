...
{
  var name;
  if(GetSpawnID()->~IsAmmoPacket())
    name = GetName(0, GetSpawnID()->~AmmoID());
  else
    name = GetName(0, GetSpawnID());
  hud->Ammo(ContentsCount(GetSpawnID()), GetMaxCount(), Format("%s (%s)", GetName(), name), true, 0, 0, source);
}