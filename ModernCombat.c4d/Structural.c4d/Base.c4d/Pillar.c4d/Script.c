...
{
  if(NoScenarioEvents()) return false;
...
{
  //Keine Szenario-Events-Regel aktiv: Grafisch unterscheiden
  if(NoScenarioEvents())
...
{
  //Bei Keine Szenario-Events-Regel: Schaden nullifizieren
  if(NoScenarioEvents())
  {
    if(iChange > 0)
      DoDamage(-iChange);
  ...
  if(GetDamage() > 100)
  {
    ...
    //Effekte
    if(GetEffectData(EFSM_ExplosionEffects) > 1) CastParticles("ConcreteSplinter",8,110,-10,0,60,100);
    if(GetEffectData(EFSM_ExplosionEffects) > 1) CastParticles("ConcreteSplinter",8,110,10,0,60,100);
