...
public func OnDeathAnnounce(object pCrew, int iKiller, int iAssist)
{
  ...
  Sound("PaperHit*.ogg", false, pCrew);
  while((iChange++) < 0)
...
private func GoalMoney(int iPlr, int iAmount)
{
  ...
  if(iAmount > 0)
  {
    //Punkte bei Belohnungssystem (Sicherung von Credits)
    DoPlayerPoints(BonusPoints("MRCredits", iAmount), RWDS_TeamPoints, GetOwner(pClonk), pClonk, IC33);

    //Zusatznachricht anzeigen
    if(pClonk)
    {
      string = Format("<c ffff33>{{IC33}} +%d</c>", iAmount);
      AddEffect("PointMessage", pClonk, 130, 1, pClonk, 0, string);
    }
  }
  else
    if(iAmount < 0 && iMoney > 0 && pClonk)
    {
      string = Format("<c ff0000>{{IC33}} %d</c>", iAmount);
      AddEffect("PointMessage", pClonk, 130, 1, pClonk, 0, string);
    }
