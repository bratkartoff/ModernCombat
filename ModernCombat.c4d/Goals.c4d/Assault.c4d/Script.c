...
public func GoalExtraValue()
{
  ...
  return Format("$Attackers$: <c %x>%s</c>", GetTeamColor(att), GetTeamName(att));
}
...
public func CreateTeamBorder(int iDefenderTeam, int iX, int iY, parameter, bool fCreateBorderObject, int iTime)
{
  ...
  border->Set(parameter, true, false, true, iDefenderTeam);
