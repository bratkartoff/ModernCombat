...
global func DoPlayerPoints(int iPoints, int iType, int iPlr, object pClonk, id idIcon, string szSound)
{
  ...
  if(db->SetPlayerData(db->GetPlayerPoints(iType, iPlr)+iPoints, iType, iPlr))
  {
    ...
    {
      if(iPoints < 50)
        sound = "RWDS_Points*.ogg";
      else
        sound = "RWDS_PointsMajor.ogg";
    }

      //Punkteanzeige entsprechend der Einstellung des Spielers
      if(pClonk->~ScoreDisplayType())
        pClonk->AddEffect("PointMessage", pClonk, 130, 1, pClonk, 0, Format("{{%i}} %s", idIcon, szMsg), sound);
      else
        pClonk->ScoreInfo(iPlr+1, Format("%s %s", szMsg, GetName(false,idIcon)), idIcon, 0, 0, "", sound);
      return true;
    }
  }
  return;
...
global func FxPointMessageStart(object pTarget, int iNo, int iTemp, string szString, string szSound)
{
  if(iTemp)
    return -1;

  EffectVar(0, pTarget, iNo) = szString;			//Nachricht
  EffectVar(1, pTarget, iNo) = szSound || "RWDS_Points*.ogg";	//Sound
...
global func FxPointMessageTimer(object pTarget, int iEffectNumber)
{
  ...
  if(!EffectVar(2,pTarget,iEffectNumber))
  {
    ...
    //Sound
    Sound(EffectVar(1, pTarget, iEffectNumber), 0, pContainer, 100, GetOwner(pTarget)+1);
