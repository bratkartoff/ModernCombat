...
static const LBRD_Overlay_BombDrop = 8;

local fRocketPod,fBombDrop;
...
public func Initialize() 
{
  ...
  SetGraphics("BombDrop", this, LHUD, LBRD_Overlay_BombDrop, GFXOV_MODE_Base);
  ...
  SetClrModulation(RGBa(255,255,255,255), this, LBRD_Overlay_BombDrop);
...
public func SetState(int iNewState, bool fKeepSound)
{
  ...
  if(fBombDrop)
  {
    SetClrModulation(dwArrowColor, this, LBRD_Overlay_BombDrop);
  }
  else
  {
    SetClrModulation(RGBa(255,255,255,255), this, LBRD_Overlay_BombDrop);
...
protected func Timer()
{
  ...
  if(fBombDrop != tBombDrop)
  {
    fUpdate = true;
    if(!fBombDrop) Sound("WarningBombDropReloaded.ogg", false, this, 100, GetOwner()+1);
    fBombDrop = tBombDrop;
  }
