...
public func FxFlyingTimer(object pTarget, int iEffectNumber, int iEffectTime)
{
  ...
  if(!(iEffectTime % 5))
  {
    ...
    if(iItemType == 0)				Sense();			//Standard: Sensorball
...
public func OnDestruction()
{
  ...
  //Verschwinden
  var obj = CreateObject(MVWK);
  SetR(GetR(), obj);
  SetXDir(GetXDir(), obj);
  SetYDir(GetYDir(), obj);
  SetRDir(RandomX(20,-20), obj);
  obj->FadeOut();
...
public func AMP(bool statusOnly)
{
  ...
  for(var pTarget in aClonks)
  {
    ...
    //Umkreis-Effekt
    ShowTransferRadius(this,TeamSupportRange());
...
public func FxWaitTimer(object pTarget, int iEffectNumber, int iEffectTime)
{
  ...
  {
    CountC4();
    if((iItemType == 0) && (GetOwner() != -1) && !(iEffectTime % 5))
      Sense();
  }
}
...
public func ControlLeft(pByObj)
{
  ...
  if(GetAction() == "Wait") return true;

  //Zielen oder Bewegung
  if(fIsAiming)
  {
    iPat_Dir = 1;
    return true;
...
protected func Collision(int iCollSpeed)
{
  ...
  //Effekte
  CastParticles("StructureSplinter",1,50,0,0,30,60);
  Sparks(Random(2)+2,RGB(255,255,Random(5)+255));
  Sound("VehicleHit*.ogg");
  DoDmg(iCollSpeed/4);
}
