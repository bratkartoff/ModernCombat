...
func Hit(int iXDir, int iYDir)
{
  //Sound
  HitSound();
  if(IsFusing() && ContainedDamage())
    Sound("GrenadeHitClose*.ogg",0,0,0,0,0,1,50);
...
{
  //Fadenkreuz über Granate mit möglicherweise neuem Winkel informieren
  var contained = Contained();
  if(contained)
  {
    var crosshair = contained->~GetCrosshair();
    if(crosshair)
