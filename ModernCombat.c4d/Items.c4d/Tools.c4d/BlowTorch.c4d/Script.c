...
{
  SetCharge(MaxEnergy(), true); //Akku geladen
...
public func Timer()
{
  //Überhitzt oder in Verwendung: Abbruch
  if(GetEffect("Overheat", this) || GetEffect("RepairObjects", this))
    return;
...
public func SetCharge(int iChange, bool fSetTo)
{
  if(!fSetTo)
    charge = BoundBy(charge+iChange,0,MaxEnergy());
  else
    charge = BoundBy(iChange,0,MaxEnergy());
}
...
public func Use(caller)
{
  ...
  else
  {
    ...
    obj = caller->FindObject2(Find_Or(Find_And(Find_Func("IsRepairable"),				//Reparierbar?
    						...
    						Find_HostileTeam(caller->~GetTeam()))),	//Feindlich?
    						Find_And(
    						Find_OCF(OCF_Alive),
    						Find_Hostile(GetOwner(caller)),
    						Find_NoContainer()),					//Nicht verschachtelt?
  ...
  if(!used)
  {
    ...
    //Effekte bei Leerlauf
    if(!Random(2))
      CreateBurnMark(10*d,-4,9,4);
  }
  else
  {
    //Effekte
...
global func FxOverheatStop(pTarget, iNo, iReason, fTemp)
{
  Sound("BWTH_Charge.ogg",0,pTarget,25);
  pTarget->~SetCharge(25, true);
}
...
func UpdateHUD(object pHUD)
{
  ...
  pHUD->Recharge(GetEffect("Overheat", this, 0, 6), 100);
}
