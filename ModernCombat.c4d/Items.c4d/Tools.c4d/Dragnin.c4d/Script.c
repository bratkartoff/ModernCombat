...
func Sting(caller)
{
  ...
  if(obj = FindObject2(Find_InRect(-10,-10,20,20),Find_OCF(OCF_CrewMember),Find_Exclude(caller),Find_Allied(GetOwner(caller)),Find_NoContainer()))
  {
    ...
    Sound("DGNN_Inject*.ogg");
    Sound("DGNN_Use.ogg", false, this, 0, GetOwner(obj) + 1);
...
func Activate(object pByObj)
{
  ...
  Sound("DGNN_Inject*.ogg");
  Sound("DGNN_Use.ogg", false, this, 0, GetOwner(pByObj) + 1);
  RemoveObject();
  return 1;
}
...
{
  //Effekt
  HealthAura(pTarget,true,iEffectTime*5);
  //Heilen
