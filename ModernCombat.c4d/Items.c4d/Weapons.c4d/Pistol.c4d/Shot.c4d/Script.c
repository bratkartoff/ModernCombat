...
func Construction(object byObj)
{
  ...
  //ID der Host-Waffe, Schützen und Team ermitteln
  wpnid = GetID(byObj);
...
public func OnBulletHit(object pObject, int iX, int iY)
{
  ...
  if(pObject)
  {
    ...
    //Bei Feind: Treffergeräusch und Fadenkreuzindikator für Schützen
...
public func HitLiquid(int iStartX, int iStartY, int iLiqX, int iLiqY)
{
  ...
  var temp = CreateObject(TIM1, x, y, NO_OWNER);	//Hilfsobjekt für Effekte
...
global func GetShooter(object weapon)
{
  ...
  //Bei Waffen den Schützen ermitteln
  if(weapon->~IsWeapon())
  {
  ...
  //Kein Schütze identifiziert: Stattdessen Waffe nehmen
  if(!shooter)
    shooter = weapon;
      return shooter;
}
