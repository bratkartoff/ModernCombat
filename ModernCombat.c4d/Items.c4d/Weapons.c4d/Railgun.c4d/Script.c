...
public func Fire1()
{
  ...
  //Effekte
  BulletCasing(x/3,y/3,-dir*Cos(angle-35*dir,40+Random(10)),-dir*Sin(angle-35*dir,40+Random(10)),7);
...
public func LaserStrike(object pObj, int iTime)
{
  ...
  //Objekt beschädigen
  if(GetCategory(pObj) & C4D_Structure || GetCategory(pObj) & C4D_Vehicle)
    DoDmg(GetFMData(FM_Damage, 1)/2, DMG_Explosion, pObj);
  else
