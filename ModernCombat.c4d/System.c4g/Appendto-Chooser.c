...
protected func OpenMenu()
{
  ...
  //Menü erstellen
  CreateMenu(GetID(),pClonk,0,4,"$GameSettings$",0,1);
  //Spielregeln
  if(!GetLeague())
    AddMenuItem("$CreateRules$", "OpenRuleMenu", CTFL, pClonk, 0,0, "$RuleInfo$");
  //Spielzieleinstellungen
  if(pGoal && pGoal->~IsConfigurable())
    AddMenuItem("%s", "OpenGoalMenu", GetID(pGoal), pClonk,0,0,"$GoalSettingsInfo$");
  if(!GetLeague())
  {
    //Teams
    if(GetTeamCount())
      AddMenuItem("$Teams$", "OpenTeamMenu", TEAM, pClonk, 0, 0, "$TeamsInfo$");
    //KI
    if(ObjectCount(WAYP))
      AddMenuItem("$AIPlayers$", "OpenAIMenu", HZCK, pClonk, 0,0, "$AIPlayersInfo$");
  }

  //Leerzeile
  AddMenuItem("", 0, 0, pClonk, 0, 0, 0, C4MN_Add_ForceNoDesc);
  ...
  if(!GetLeague())
  {
    ...
    //Spielzieländerung
    AddMenuItem("$ChangeGoal$", "ChangeGoal", CHOS, pClonk, 0, 0, "$ChangeGoal$");
  }
  //Menüweitergabe
  ...
  //Runde starten
  AddMenuItem("$StartRound$", "ConfigurationFinished", GOCC, pClonk,0,pClonk, 0, C4MN_Add_ImgIndexed | C4MN_Add_ForceNoDesc, 3);
}
...
protected func ChangeGoal(id dummy, int selection)
{
  //Existierende Spielziele entfernen
  ...
  EventInfo4K(0, "$GoalChanges$", CHOS, 0, 0, 0, "Info_Event.ogg");

  aGoals = aGoalCache;

  //Spielzielauswahlmenü öffnen
  OpenGoalChooseMenu();
}
...
protected func SetGoalDesc(id idGoal, string szOptMessage)
{
  var desc, str;
  if(szOptMessage)
    desc = szOptMessage;
  ...
  {
    desc = pGoal->GoalDescription();
    if(!desc)
      desc = GetDesc(pGoal);
  }
  str = Format("@{{%i}}<c ffff00>%s</c>|%s",idGoal,GetName(pGoal),desc);
...
protected func OpenGoalChooseMenu()
{
  ...
  //Spielziele für Spielzielneuwahl cachen
  aGoalCache = aGoals;
  ...
  //Menü erstellen
  CreateMenu(GetID(),pClonk,0,4,"$GoalSelection$",0,1);
  ...
  AddMenuItem("$ChangeHost$", "ChangeHostMenu", MCMC, pClonk, 0,0, "$ChangeHostInfo$");
  //Zufallswahl
  AddMenuItem("$RandomGoal$", "OpenGoalRandomMenu", GetID(), pClonk, 0, pClonk, "$RandomGoalInfo$");
  //Abstimmung
  AddMenuItem("$VoteGoal$", "CheckGoalVoteMenu", GetID(), pClonk, 0, pClonk, "$VoteGoalInfo$");
}
...
public func ChangeHostMenu()
{
  ...
  //Menü erstellen
  CreateMenu(GetID(),pClonk,0,4,"$ChangeHost$",0,1);
  ...
  for(var i = 0; i < GetPlayerCount(C4PT_User); i++)
  {
    ...
    AddMenuItem(Format("<c %x>%s</c>", clr, name), cmd, 0, pClonk, 0, plr, 0, C4MN_Add_ForceNoDesc);
  }

  AddMenuItem("$Back$", "OpenMenu", 0, pClonk, 0, 0, 0, C4MN_Add_ForceNoDesc);
  return true;
}
...
private func PopulateGoalSelectionMenu(object pClonk, string function, int offset)
{
  var rGoals = RecommendedGoals();
  if(!rGoals)
    rGoals = [];
  ...
  for(var i = 0; i < GetLength(aGoals); i++)
  {
    ...
    if(aGoalsChecked[i])
    {
      AddMenuItem("%s", Format("CheckGoal(%i, Object(%d), \"%s\", %d)", aGoals[i], pClonk->ObjectNumber(), function, offset), aGoals[i], pClonk, 0, 0, GetDesc(0, aGoals[i]), 4, obj);
      ++checked;
    }
    else
      AddMenuItem("<c 777777>%s</c>", Format("CheckGoal(%i, Object(%d), \"%s\", %d)", aGoals[i], pClonk->ObjectNumber(), function, offset), aGoals[i], pClonk, 0, 0, GetDesc(0, aGoals[i]), 4, obj);
...
protected func CheckGoal(id idGoal, object pClonk, string function, int offset)
{
  var iIndex = GetIndexOf(idGoal, aGoals);
  aGoalsChecked[iIndex] = !aGoalsChecked[iIndex];
  Call(function, 0, pClonk);
  SelectMenuItem(iIndex + offset, pClonk);
...
{
  //Menü erstellen
  CreateMenu(GetID(),pClonk,0,4,"$RandomGoal$",0,1);

  AddMenuItem("<c 777777>$BeginRandomGoal$</c>", 0, GetID(), pClonk, 0, 0, 0, C4MN_Add_ForceNoDesc);
  var result = PopulateGoalSelectionMenu(pClonk, "OpenGoalRandomMenu", 1);
  if(result[1] >= 2)
    AddMenuItem("$Yes$", "GoalRandomChoose", GOCC, pClonk, 0, pClonk, 0, C4MN_Add_ImgIndexed | C4MN_Add_ForceNoDesc, 3);
  else
    AddMenuItem("<c 777777>$Yes$</c>", 0, GOCC, pClonk, 0, pClonk, 0, C4MN_Add_ImgIndexed | C4MN_Add_ForceNoDesc, 3);
  AddMenuItem("$Back$", "OpenGoalChooseMenu", 0, pClonk, 0, 0, 0, C4MN_Add_ForceNoDesc);

  pClonk->SelectMenuItem(result[0] + 1);
}
...
{
  //Menü erstellen
  CreateMenu(GetID(),pClonk,0,4,"$VoteGoal$",0,1);
  AddMenuItem("<c 777777>$BeginVote$</c>", 0, GetID(), pClonk, 0, 0, 0, C4MN_Add_ForceNoDesc);
  var result = PopulateGoalSelectionMenu(pClonk, "CheckGoalVoteMenu", 1);
  if(result[1] >= 2)
    AddMenuItem("$Yes$", "OpenGoalVoteMenu", GOCC, pClonk, 0, pClonk, 0, C4MN_Add_ImgIndexed | C4MN_Add_ForceNoDesc, 3);
  else
    AddMenuItem("<c 777777>$Yes$</c>", 0, GOCC, pClonk, 0, pClonk, 0, C4MN_Add_ImgIndexed | C4MN_Add_ForceNoDesc, 3);
  AddMenuItem("$Back$", "OpenGoalChooseMenu", 0, pClonk, 0, 0, 0, C4MN_Add_ForceNoDesc);
...
protected func OpenGoalVoteMenu(id id, object pClonk)
{
  if(!aGoalsVoted)
    aGoalsVoted = [];
  //Wahlmenü für jeden Spieler öffnen
  for(var i = 0; i < GetPlayerCount(); i++)
  {
    aGoalsVoted[i] = [];
...
protected func GoalVoteMenu(id id, object pClonk, int iPlr)
{
  ...
  for(var i = 0; i < GetLength(aGoals); i++)
  {
    ...
    if(aGoalsVoted[iPlr][i])
      AddMenuItem("%s", Format("CheckVoteGoal(%i, Object(%d), %d)", aGoals[i], pClonk->ObjectNumber(), item++), aGoals[i], pClonk, 0, 0, GetDesc(0, aGoals[i]), 4, obj);
    else
      AddMenuItem("<c 777777>%s</c>", Format("CheckVoteGoal(%i, Object(%d), %d)", aGoals[i], pClonk->ObjectNumber(), item++), aGoals[i], pClonk, 0, 0, GetDesc(0, aGoals[i]), 4, obj);
...
protected func CheckVoteGoal(id idGoal, object pClonk, int selection)
{
