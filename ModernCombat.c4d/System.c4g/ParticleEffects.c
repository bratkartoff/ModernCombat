...
global func AmmoAura(object pTarget, bool fNoParticle)
{
  var x = GetX(pTarget)-GetX();
  var y = GetY(pTarget)-GetY();
  var height = GetObjHeight(pTarget);
  var randomA = Random(10);
  var randomB = Random(10);

  //Effekte
  CreateParticle("ShockWave", x, y, randomA, randomB, 5*(5+height), RGB(255,204,0), pTarget);
  CreateParticle("ShockWave", x, y, randomA, randomB, 5*(10+height), RGB(255,204,0), pTarget);
  CreateParticle("ShockWave", x, y, randomA, randomB, 5*(15+height), RGB(255,204,0), pTarget);
  if(!Contained(pTarget) && !fNoParticle) CastParticles("BulletCasing", RandomX(2,4), 30, x, y, 20, 35);
...
global func HealthAura(object pTarget, bool fNoParticle, int iModifier)
{
  var x = GetX(pTarget)-GetX();
  var y = GetY(pTarget)-GetY();
  var height = GetObjHeight(pTarget);
  ...
  //Effekte
  if(iModifier)
    CreateParticle("ShockWave", x, y, randomA, randomB, 5*height+25+Sin(iModifier,35), RGB(0,230,255), pTarget);
  else
  {
    CreateParticle("ShockWave", x, y, randomA, randomB, 5*(5+height), RGB(0,230,255), pTarget);
    CreateParticle("ShockWave", x, y, randomA, randomB, 5*(10+height), RGB(0,230,255), pTarget);
    CreateParticle("ShockWave", x, y, randomA, randomB, 5*(15+height), RGB(0,230,255), pTarget);
  }
  if(!Contained(pTarget) && !fNoParticle) CastParticles("Paper", RandomX(1,3), 10, x, y, 15, 30);
